...
Локально запросы к GREEN-API проксирует Vite. На Netlify тот же путь `/green-api` проксирует `netlify.toml`. Ожидание входящего сообщения — 15 секунд: прокси Netlify обрывает запрос через 26 секунд.
